...
function testAjax(tag) { //添加标注后调用

	var users = {'content': $(".message").html(), 'tag': tag};
...
//							alert($(this).find("th:third-child").html());
	...
	$(".button").click(function(){
		...
		if(document.selection) {
	    	...
	    	var txt = selection.getRangeAt(0);

	    	
	    	var para = document.getElementsByTagName('span');
	        for (var i = 0;i<para.length;i++){
	            if(selection.containsNode(para[i],true)){
	            alert("请勿重复标注");
	            return false;         
	           }
	        }
...
$(".label-submit").click(function(){
	//var text ={'content': $(".message").html()};

	alert($('#submit-form').serialize());
	$.ajax({
...
$(".button-submitedit").click(function(){/*
		$.ajax({
			type:'post',
			datatype:'text',
			url:"http://localhost:8080/annotationTool/biaozhu",
	...
	$.ajax({
		...
		url :'/annotationTool/biaozhu',

		success :function(data) {
			//window.location.href='views/entity/label';
